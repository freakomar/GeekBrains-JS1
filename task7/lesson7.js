...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        scoreView.innerHTML = "Текущий счет: " + score;
    }
    return check;
}
...
function createFood() {
    ...
    while (!foodCreated) { //пока еду не создали
        ...
        // проверка на змейку
        if (!food_cell_classes.includes('snake-unit') || !food_cell_classes.includes('bomb')) {
            var classes = '';
...
function createBomb() {
    ...
    while (!bombCreated) { //пока бомбу не создали
        ...
        if (!bomb_cell_classes.includes('snake-unit') || !bomb_cell_classes.includes('food-unit')) {
            ...
            bomb_cell.setAttribute('class', classes + 'bomb');
            bombCreated = true;
        }
    }
}
